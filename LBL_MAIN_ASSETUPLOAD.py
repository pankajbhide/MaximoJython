...
def getSynonymDomainDesc(strDomain, strValue,runAsUserInfo1):
    strDomainDesc=None
    DomainsSet = MXServer.getMXServer().getMboSet("synonymdomain", runAsUserInfo1)
    DomainsSet.setUserWhere("domainid='" + strDomain+"' and maxvalue='FACILITIES' and value='" + strValue +"'" )
    if (not DomainsSet.isEmpty()):
                strDomainDesc=DomainsSet.getMbo(0).getString("description")
    DomainsSet=None      
    return strDomainDesc
...
assetupd_detailsSetRemote.setUserWhere(" status is null order by lbl_assetupd_detailsid") # existing record
        ...
        if (isBlank(strAssetnum) != True):
            ...
            if (strDescription != assetupd_detailsSetRemote.getMbo(i).getString("description")):
                if (isBlank(assetupd_detailsSetRemote.getMbo(i).getString("description"))== True):
                    ...
                    insert_asset_log(lbl_assetupd_logRemote,runAsUserInfo1,strAssetnum,strAssetnum,"WARNING","Asset description should not be blank")
                else:
            ...
            '''
            ##################################################
            # Additional fields and validations Oct 24, 2017
            ################################################# 
            strAssetType=assetupd_detailsSetRemote.getMbo(i).getString("assettype")
            if (isBlank(strAssetType)==False):
                strTemp=getSynonymDomainDesc("ASSETTYPE", strAssetType, runAsUserInfo1)
                if (isBlank(strTemp) == True):
                    
                    boolError=True
                    insert_asset_log(lbl_assetupd_logRemote,runAsUserInfo1,strAssetnum,strAssetnum,"ERROR","Invalid asset type " + strAssetType)
                else:
                    assetSet.setValue("assettype", strAssetType,  MboConstants.NOACCESSCHECK | MboConstants.NOVALIDATION_AND_NOACTION)
                    
            ################################################# 
            strAssetSystem=assetupd_detailsSetRemote.getMbo(i).getString("lbl_assetsystem")
            if (isBlank(strAssetSystem)==False):
                strTemp=getDomainDesc("LBL_ASSETSYSTEM", strAssetSystem, runAsUserInfo1)
                if (isBlank(strTemp) == True):
                    
                    boolError=True
                    insert_asset_log(lbl_assetupd_logRemote,runAsUserInfo1,strAssetnum,strAssetnum,"ERROR","Invalid asset system " + strAssetSystem)
                else:
                    assetSet.setValue("lbl_assetsystem", strAssetSystem,  MboConstants.NOACCESSCHECK | MboConstants.NOVALIDATION_AND_NOACTION)
           
                          
            strAssetUnit=assetupd_detailsSetRemote.getMbo(i).getString("lbl_unitnum")
            if (isBlank(strAssetUnit)==True):  
                boolError=True
                insert_asset_log(lbl_assetupd_logRemote,runAsUserInfo1,strAssetnum,strAssetnum,"ERROR","Asset unit should not be null" + strAssetSystem)
            else:
                assetSet.setValue("lbl_unitnum", strAssetUnit,  MboConstants.NOACCESSCHECK | MboConstants.NOVALIDATION_AND_NOACTION)
